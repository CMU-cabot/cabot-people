...
SNGNN_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SNGNN2D-v2')
...
class SNGNNNode(Node):
    ...
    def _process(self):
        ...
        try:
            ...
            self.get_logger().info("Published sngnn_costmap")
            ...
            if self.save_image:
                ...
                if now_sec - self.last_save_time >= self.save_interval_sec:
                    ...
                    try:
                        ...
                        self.get_logger().info(f"Saved debug data to {out_path}")
                    except Exception as e:
